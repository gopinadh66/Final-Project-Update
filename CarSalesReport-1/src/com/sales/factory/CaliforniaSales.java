...
public class CaliforniaSales  implements Car {
	...
	public void consolidateReport() {
		System.out.println("                                      California Car Sales Report" );
		SalesCommand oldSales = new OldCarSales();
		SalesCommand newSales = new NewCarSales();
		CaliforniaSales tOldSales = new CaliforniaSales();
		CaliforniaSales tNewSales = new CaliforniaSales();
		...
		for(String model:getModelsList()) {
			String oldData = oldSales.execute("California", model);
			String newData = newSales.execute("California", model);
			...
			tOldSales.addSales(new California(model,Integer.parseInt(oldSplitData[0]),Double.parseDouble(oldSplitData[1])));
			tNewSales.addSales(new California(model,Integer.parseInt(newSplitData[0]),Double.parseDouble(newSplitData[1])));
		}
